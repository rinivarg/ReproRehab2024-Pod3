{
 "cells": [
  {
   "cell_type": "markdown",
   "id": "37041a78-063f-4aab-8db6-7607f728fb91",
   "metadata": {},
   "source": [
    "# Week 1: Getting started with R\n",
    "If you haven't yet downloaded and installed R, now is the time to do so. <br>\n",
    "Go to the Resource Links to be directed"
   ]
  },
  {
   "cell_type": "markdown",
   "id": "ec95d909-508e-4b18-8c4a-7c7f2814a56a",
   "metadata": {},
   "source": [
    "## Clear the entire workspace\n",
    "\n",
    "The function `rm(list=ls())` in R is used to **clear the entire workspace** by removing all objects from the environment.\n",
    "\n",
    "Here’s how it works:\n",
    "\n",
    "1. `ls()` lists all the objects (variables, functions, data frames, etc.) currently stored in the R environment.\n",
    "2. `rm(list=...)` removes the specified objects from the environment. By passing `ls()` as the argument to `rm()`, you're telling R to remove all the objects listed by `ls()`, which effectively clears the workspace.\n",
    "\n",
    "In short, it deletes all objects, giving you a clean slate."
   ]
  },
  {
   "cell_type": "code",
   "execution_count": 4,
   "id": "c6219fd5-fddc-4c07-a04b-ecde95b88777",
   "metadata": {
    "vscode": {
     "languageId": "r"
    }
   },
   "outputs": [],
   "source": [
    "rm(list=ls())"
   ]
  },
  {
   "cell_type": "markdown",
   "id": "3dd71e91-a353-4137-b64d-74866d94dd16",
   "metadata": {},
   "source": [
    "## Install and Load the Packages you need\n",
    "the two lines here load the required libraries specified by the variable **ReqdLibs** that you define.\n",
    "\n",
    "Here's how it works:\n",
    "\n",
    "1. **`ReqdLibs = c(\"here\", \"ggplot2\", \"dplyr\")`** creates a vector (`ReqdLibs`) that contains the names of the libraries (packages) you want to load: `here`, `ggplot2`, and `dplyr`.\n",
    "\n",
    "2. **`lapply(ReqdLibs, library, character.only = TRUE)`** loops over each item in `ReqdLibs` and applies the function (`library`) to load each package.\n",
    "The **`character.only = TRUE`** part tells `library()` to treat the package names as strings, rather than as variable names. This is necessary since the package names are passed as strings in the vector.\n",
    "\n",
    "You've now loaded the 3 packages *here*, *ggplto2* and *dplyr*\n",
    "Note that this only works if you've installed these pacakges previously. Suppose you hadn't installed any of them,can you think of how you modify the second line of this code to \"install.packages\"?"
   ]
  },
  {
   "cell_type": "code",
   "execution_count": 9,
   "id": "bf7e040a-4a93-445e-b548-48c83bc72b8a",
   "metadata": {
    "vscode": {
     "languageId": "r"
    }
   },
   "outputs": [
    {
     "data": {
      "text/html": [
       "<ol>\n",
       "\t<li><style>\n",
       ".list-inline {list-style: none; margin:0; padding: 0}\n",
       ".list-inline>li {display: inline-block}\n",
       ".list-inline>li:not(:last-child)::after {content: \"\\00b7\"; padding: 0 .5ex}\n",
       "</style>\n",
       "<ol class=list-inline><li>'dplyr'</li><li>'ggplot2'</li><li>'here'</li><li>'repr'</li><li>'stats'</li><li>'graphics'</li><li>'grDevices'</li><li>'utils'</li><li>'datasets'</li><li>'methods'</li><li>'base'</li></ol>\n",
       "</li>\n",
       "\t<li><style>\n",
       ".list-inline {list-style: none; margin:0; padding: 0}\n",
       ".list-inline>li {display: inline-block}\n",
       ".list-inline>li:not(:last-child)::after {content: \"\\00b7\"; padding: 0 .5ex}\n",
       "</style>\n",
       "<ol class=list-inline><li>'dplyr'</li><li>'ggplot2'</li><li>'here'</li><li>'repr'</li><li>'stats'</li><li>'graphics'</li><li>'grDevices'</li><li>'utils'</li><li>'datasets'</li><li>'methods'</li><li>'base'</li></ol>\n",
       "</li>\n",
       "\t<li><style>\n",
       ".list-inline {list-style: none; margin:0; padding: 0}\n",
       ".list-inline>li {display: inline-block}\n",
       ".list-inline>li:not(:last-child)::after {content: \"\\00b7\"; padding: 0 .5ex}\n",
       "</style>\n",
       "<ol class=list-inline><li>'dplyr'</li><li>'ggplot2'</li><li>'here'</li><li>'repr'</li><li>'stats'</li><li>'graphics'</li><li>'grDevices'</li><li>'utils'</li><li>'datasets'</li><li>'methods'</li><li>'base'</li></ol>\n",
       "</li>\n",
       "</ol>\n"
      ],
      "text/latex": [
       "\\begin{enumerate}\n",
       "\\item \\begin{enumerate*}\n",
       "\\item 'dplyr'\n",
       "\\item 'ggplot2'\n",
       "\\item 'here'\n",
       "\\item 'repr'\n",
       "\\item 'stats'\n",
       "\\item 'graphics'\n",
       "\\item 'grDevices'\n",
       "\\item 'utils'\n",
       "\\item 'datasets'\n",
       "\\item 'methods'\n",
       "\\item 'base'\n",
       "\\end{enumerate*}\n",
       "\n",
       "\\item \\begin{enumerate*}\n",
       "\\item 'dplyr'\n",
       "\\item 'ggplot2'\n",
       "\\item 'here'\n",
       "\\item 'repr'\n",
       "\\item 'stats'\n",
       "\\item 'graphics'\n",
       "\\item 'grDevices'\n",
       "\\item 'utils'\n",
       "\\item 'datasets'\n",
       "\\item 'methods'\n",
       "\\item 'base'\n",
       "\\end{enumerate*}\n",
       "\n",
       "\\item \\begin{enumerate*}\n",
       "\\item 'dplyr'\n",
       "\\item 'ggplot2'\n",
       "\\item 'here'\n",
       "\\item 'repr'\n",
       "\\item 'stats'\n",
       "\\item 'graphics'\n",
       "\\item 'grDevices'\n",
       "\\item 'utils'\n",
       "\\item 'datasets'\n",
       "\\item 'methods'\n",
       "\\item 'base'\n",
       "\\end{enumerate*}\n",
       "\n",
       "\\end{enumerate}\n"
      ],
      "text/markdown": [
       "1. 1. 'dplyr'\n",
       "2. 'ggplot2'\n",
       "3. 'here'\n",
       "4. 'repr'\n",
       "5. 'stats'\n",
       "6. 'graphics'\n",
       "7. 'grDevices'\n",
       "8. 'utils'\n",
       "9. 'datasets'\n",
       "10. 'methods'\n",
       "11. 'base'\n",
       "\n",
       "\n",
       "\n",
       "2. 1. 'dplyr'\n",
       "2. 'ggplot2'\n",
       "3. 'here'\n",
       "4. 'repr'\n",
       "5. 'stats'\n",
       "6. 'graphics'\n",
       "7. 'grDevices'\n",
       "8. 'utils'\n",
       "9. 'datasets'\n",
       "10. 'methods'\n",
       "11. 'base'\n",
       "\n",
       "\n",
       "\n",
       "3. 1. 'dplyr'\n",
       "2. 'ggplot2'\n",
       "3. 'here'\n",
       "4. 'repr'\n",
       "5. 'stats'\n",
       "6. 'graphics'\n",
       "7. 'grDevices'\n",
       "8. 'utils'\n",
       "9. 'datasets'\n",
       "10. 'methods'\n",
       "11. 'base'\n",
       "\n",
       "\n",
       "\n",
       "\n",
       "\n"
      ],
      "text/plain": [
       "[[1]]\n",
       " [1] \"dplyr\"     \"ggplot2\"   \"here\"      \"repr\"      \"stats\"     \"graphics\" \n",
       " [7] \"grDevices\" \"utils\"     \"datasets\"  \"methods\"   \"base\"     \n",
       "\n",
       "[[2]]\n",
       " [1] \"dplyr\"     \"ggplot2\"   \"here\"      \"repr\"      \"stats\"     \"graphics\" \n",
       " [7] \"grDevices\" \"utils\"     \"datasets\"  \"methods\"   \"base\"     \n",
       "\n",
       "[[3]]\n",
       " [1] \"dplyr\"     \"ggplot2\"   \"here\"      \"repr\"      \"stats\"     \"graphics\" \n",
       " [7] \"grDevices\" \"utils\"     \"datasets\"  \"methods\"   \"base\"     \n"
      ]
     },
     "metadata": {},
     "output_type": "display_data"
    }
   ],
   "source": [
    "ReqdLibs = c(\"here\",\"ggplot2\",\"dplyr\")\n",
    "lapply(ReqdLibs, library, character.only = TRUE)"
   ]
  },
  {
   "cell_type": "markdown",
   "id": "5142b783-0799-4240-a622-f7c07156a29e",
   "metadata": {},
   "source": [
    "## So, where are you? Locating your working directory\n",
    "now, we will use the 'here' function in the 'here' package to do this.\n",
    "* The here() function in R helps you locate files and directories in a project in a reliable and reproducible way. It is particularly useful for avoiding issues with file paths when working across different systems or environments.\n",
    "* Note that the output of the 'here' function is of character type. In other words, it will print out the root folder path, not link you to the path (see more next)"
   ]
  },
  {
   "cell_type": "code",
   "execution_count": 22,
   "id": "386201d0-4c6e-4981-9676-4f8ba889ce88",
   "metadata": {
    "vscode": {
     "languageId": "r"
    }
   },
   "outputs": [
    {
     "data": {
      "text/html": [
       "'/Users/rinivarghese/Documents/My Documents/JHU/Lab/Professional Development/ReproRehab 2024/ReproRehab_Bootcamp'"
      ],
      "text/latex": [
       "'/Users/rinivarghese/Documents/My Documents/JHU/Lab/Professional Development/ReproRehab 2024/ReproRehab\\_Bootcamp'"
      ],
      "text/markdown": [
       "'/Users/rinivarghese/Documents/My Documents/JHU/Lab/Professional Development/ReproRehab 2024/ReproRehab_Bootcamp'"
      ],
      "text/plain": [
       "[1] \"/Users/rinivarghese/Documents/My Documents/JHU/Lab/Professional Development/ReproRehab 2024/ReproRehab_Bootcamp\""
      ]
     },
     "metadata": {},
     "output_type": "display_data"
    },
    {
     "data": {
      "text/html": [
       "'character'"
      ],
      "text/latex": [
       "'character'"
      ],
      "text/markdown": [
       "'character'"
      ],
      "text/plain": [
       "[1] \"character\""
      ]
     },
     "metadata": {},
     "output_type": "display_data"
    }
   ],
   "source": [
    "# working directory root\n",
    "folder_path = here()\n",
    "folder_path\n",
    "\n",
    "# check the type of variable file_path is\n",
    "class(folder_path)"
   ]
  },
  {
   "cell_type": "markdown",
   "id": "59063f41-0396-43bb-a348-8b0a8d4fd130",
   "metadata": {},
   "source": [
    "## Calling a specific subdirectory\n",
    "Suppose that you now want to append this folder path with a subdirectory called \"Subject_1\" and within it you want to call a specific file, say data.csv file, then the 'here' function can help append the correct root directory up to that point in the file path."
   ]
  },
  {
   "cell_type": "code",
   "execution_count": 25,
   "id": "cc9fe5e9-431c-460a-96e8-a164529968cd",
   "metadata": {
    "vscode": {
     "languageId": "r"
    }
   },
   "outputs": [
    {
     "data": {
      "text/html": [
       "'/Users/rinivarghese/Documents/My Documents/JHU/Lab/Professional Development/ReproRehab 2024/ReproRehab_Bootcamp/data/data.csv'"
      ],
      "text/latex": [
       "'/Users/rinivarghese/Documents/My Documents/JHU/Lab/Professional Development/ReproRehab 2024/ReproRehab\\_Bootcamp/data/data.csv'"
      ],
      "text/markdown": [
       "'/Users/rinivarghese/Documents/My Documents/JHU/Lab/Professional Development/ReproRehab 2024/ReproRehab_Bootcamp/data/data.csv'"
      ],
      "text/plain": [
       "[1] \"/Users/rinivarghese/Documents/My Documents/JHU/Lab/Professional Development/ReproRehab 2024/ReproRehab_Bootcamp/data/data.csv\""
      ]
     },
     "metadata": {},
     "output_type": "display_data"
    }
   ],
   "source": [
    "file_path = here(\"data\", \"data.csv\")\n",
    "file_path"
   ]
  },
  {
   "cell_type": "markdown",
   "id": "78b1c65a-675e-4a7a-8394-513d15c23c54",
   "metadata": {},
   "source": [
    "## Last step: list packages in use\n",
    "it is good practice to wrap up a codebook by listing the pacakges that were being used in it... even the ones we didn't call explicitly call in the first line but were there by default (e.g., base R)"
   ]
  },
  {
   "cell_type": "markdown",
   "id": "59486c61-00b0-4177-82aa-c5912d203d2a",
   "metadata": {},
   "source": []
  },
  {
   "cell_type": "code",
   "execution_count": 29,
   "id": "6b05f390-c2a3-4873-8cb2-6477be06b97a",
   "metadata": {
    "vscode": {
     "languageId": "r"
    }
   },
   "outputs": [
    {
     "data": {
      "text/html": [
       "<style>\n",
       ".list-inline {list-style: none; margin:0; padding: 0}\n",
       ".list-inline>li {display: inline-block}\n",
       ".list-inline>li:not(:last-child)::after {content: \"\\00b7\"; padding: 0 .5ex}\n",
       "</style>\n",
       "<ol class=list-inline><li>'.GlobalEnv'</li><li>'package:dplyr'</li><li>'package:ggplot2'</li><li>'package:here'</li><li>'package:repr'</li><li>'jupyter:irkernel'</li><li>'package:stats'</li><li>'package:graphics'</li><li>'package:grDevices'</li><li>'package:utils'</li><li>'package:datasets'</li><li>'package:methods'</li><li>'Autoloads'</li><li>'package:base'</li></ol>\n"
      ],
      "text/latex": [
       "\\begin{enumerate*}\n",
       "\\item '.GlobalEnv'\n",
       "\\item 'package:dplyr'\n",
       "\\item 'package:ggplot2'\n",
       "\\item 'package:here'\n",
       "\\item 'package:repr'\n",
       "\\item 'jupyter:irkernel'\n",
       "\\item 'package:stats'\n",
       "\\item 'package:graphics'\n",
       "\\item 'package:grDevices'\n",
       "\\item 'package:utils'\n",
       "\\item 'package:datasets'\n",
       "\\item 'package:methods'\n",
       "\\item 'Autoloads'\n",
       "\\item 'package:base'\n",
       "\\end{enumerate*}\n"
      ],
      "text/markdown": [
       "1. '.GlobalEnv'\n",
       "2. 'package:dplyr'\n",
       "3. 'package:ggplot2'\n",
       "4. 'package:here'\n",
       "5. 'package:repr'\n",
       "6. 'jupyter:irkernel'\n",
       "7. 'package:stats'\n",
       "8. 'package:graphics'\n",
       "9. 'package:grDevices'\n",
       "10. 'package:utils'\n",
       "11. 'package:datasets'\n",
       "12. 'package:methods'\n",
       "13. 'Autoloads'\n",
       "14. 'package:base'\n",
       "\n",
       "\n"
      ],
      "text/plain": [
       " [1] \".GlobalEnv\"        \"package:dplyr\"     \"package:ggplot2\"  \n",
       " [4] \"package:here\"      \"package:repr\"      \"jupyter:irkernel\" \n",
       " [7] \"package:stats\"     \"package:graphics\"  \"package:grDevices\"\n",
       "[10] \"package:utils\"     \"package:datasets\"  \"package:methods\"  \n",
       "[13] \"Autoloads\"         \"package:base\"     "
      ]
     },
     "metadata": {},
     "output_type": "display_data"
    }
   ],
   "source": [
    "search()"
   ]
  },
  {
   "cell_type": "markdown",
   "id": "fe509e86-ea1e-4896-96a7-444dcf21e467",
   "metadata": {},
   "source": [
    "## Resource Links"
   ]
  },
  {
   "cell_type": "markdown",
   "id": "570b79f6-038c-431b-9996-c06e9ace097a",
   "metadata": {},
   "source": [
    "1) Downloading and Installing R & R studio\n",
    "   * https://posit.co/download/rstudio-desktop/\n",
    "2) The *here* package:\n",
    "    * http://jenrichmond.rbind.io/post/how-to-use-the-here-package/\n",
    "    * http://jenrichmond.rbind.io/post/where-is-here/"
   ]
  },
  {
   "cell_type": "markdown",
   "id": "02b3b995-9139-4b37-a09c-0d1f087ccc21",
   "metadata": {},
   "source": [
    "## The End"
   ]
  }
 ],
 "metadata": {
  "kernelspec": {
   "display_name": "R",
   "language": "R",
   "name": "ir"
  },
  "language_info": {
   "codemirror_mode": "r",
   "file_extension": ".r",
   "mimetype": "text/x-r-source",
   "name": "R",
   "pygments_lexer": "r",
   "version": "4.4.1"
  }
 },
 "nbformat": 4,
 "nbformat_minor": 5
}
